...
.text
	...
	# Βρόνχος στον οποίο με την χρήση συναρτήσεων ζητούνται δύο αριθμοί από τον χρήστη, εκτελούμε τον αλγόριθμο του Ευκλείδη και τυπώνουμε το αποτέλεσμα
	loop:			
		# Αύξηση του μετρητή επαναλήψεων κατά 1
		add $t2, $t2, 1         # $t2 = $t2 + 1
		...
		bltz $t1,if_t1_negative # Aν $t1 < 0 κάνε άλμα στο if_t1_negative
	...
	if_t0_not_zero:
		...
		j print_result   # H συνάρτηση print_result τυπώνει τον καταχωρητή $t0 στον οποίο βρίσκεται το αποτέλεσμα, οπότε απλά κάνουμε άλμα σε αυτήν

	# Σε περίπτωση όπου η τιμή του $t1 δεν είναι μηδέν, ελένχουμε αν η τιμή του $t0 είναι. Αν ναι τότε το $t1 είναι το αποτέλεσμα			
	if_t1_not_zero:
		move $t0,$t1   # Αποθήκευση της τιμής του καταχωρητή $t1 στον καταχωρητή $t0 καθώς η συνάρτηση print_result τυπώνει την τιμή του καταχωρητή $t0
	...
	euclid_loop:		
		div $t0, $t1           # Ακέραια διαίρεση των τιμών των καταχωρητών $t0, $t1 και αποθήκευση του πηλίκου της διαίρεσης στον καταχωρητή Lo και του υπολοίπου στον καταχωρητή Hi	
		...
		bnez $t1,euclid_loop   # Αν $t1 != 0 κάνε άλμα στο euclid_loop 		
