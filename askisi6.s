#Γέροντας Αλέξανδρος icsd15029

.data																#Γίνεται παρακάτω δήλωση των string.
str: .asciiz " Give temperature "
str1: .asciiz " Dwse to plithos ton thermokrasion:  "
str3: .asciiz ": "
str2: .asciiz "\n >> Kinoumenoi mesoi:    "
str4: .asciiz " "
...
main:
	...
	get_values:
		...
		syscall
		la $a0, str3 												#Καταχώρηση string στον καταχωρητή $a0.
		li $v0, 4													#Εμφάνιση string στην κονσόλα του QTSPIM.
		syscall
		li $v0, 5													#Ανάγνωση ακεραίου από το χρήστη για την πρώτη τιμή της ακολουθίας.
		syscall
	...
	div $t4,$t4,$t6								
	la $a0, str5 													#Καταχώρηση string στον καταχωρητή $a0.
	li $v0, 4														#Εμφάνιση string στην κονσόλα του QTSPIM.
	syscall
	li $v0, 1														#Eμφάνιση αθροίσματος.
	move $a0, $t1													#Εμφάνιση της τιμής που περιέχει ο $t2.
	syscall		
	la $a0, str6 													#Καταχώρηση string στον καταχωρητή $a0.
	li $v0, 4														#Εμφάνιση string στην κονσόλα του QTSPIM.
	syscall
	li $v0, 1														#Eμφάνιση αθροίσματος.
...
li $v0, 10 															#Tερματισμός του προγράμματος.									
		syscall	
